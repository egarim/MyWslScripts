...
# - Fix: supplies SEQ_FIRSTRUN_ADMINPASSWORD on first-run
# Author: Production Deployment Script
# Version: 1.1 (Admin PW + Robust Proxy & Wait)
# Date: August 2025
set -euo pipefail
...
DEFAULT_SEQ_ADMIN_PASSWORD="ChangeMe!123"  # used only on first run
...
prompt_inputs(){
  ...
  read -p "Initial admin password (first run only) [$DEFAULT_SEQ_ADMIN_PASSWORD]: " SEQ_ADMIN_PASSWORD; SEQ_ADMIN_PASSWORD=${SEQ_ADMIN_PASSWORD:-$DEFAULT_SEQ_ADMIN_PASSWORD}
  ...
  echo "  TCP ingest port:   $TCP_INGEST_PORT (host -> container:5341)"
  echo "  Open TCP ingest:   $OPEN_TCP_INGEST ${TCP_INGEST_CIDR:+($TCP_INGEST_CIDR)}"
  echo "  Accept EULA:       $ACCEPT_EULA"
  echo "  Image:             $SEQ_IMAGE"
  echo "  SSL now:           $ENABLE_SSL"
  echo "  Admin password:    (hidden; used only if data dir is new)"
  echo
...
run_seq_container(){
  ...
  # Detect if data dir already has content; if empty, it's first run.
  local first_run="false"
  if [ -z "$(ls -A "$DATA_DIR" 2>/dev/null)" ]; then
    first_run="true"
    warn "Data directory appears empty: will apply FIRST RUN settings"
  else
    log "Data directory is not empty: assuming Seq already initialized (first-run settings will be ignored by Seq)"
  fi
  ...
  # Only add first-run admin password env if the data dir is empty
...
wait_for_seq(){
  local port="$1"; local max=60; local n=1
  log "Waiting for Seq at http://127.0.0.1:${port}/ ..."
  until curl -fsS "http://127.0.0.1:${port}/" >/dev/null 2>&1; do
    if (( n >= max )); then
      warn "Seq did not respond after ~$((max*2))s; continuing (UI may 503 until ready)"
      return
    fi
    sleep 2; n=$((n+1))
  done
